...
################################################################
    ...
    def _set_rec_init(self, seg, conn, port):
        seg['records'][conn] = {
            'last_time': 0,
            'port': port,
    ...
    def _manage_segment(self, datapath, conn, port):
        dpid = datapath.id
        ...
        if conn not in rec_t and conn not in rec_p:
            ...
            self._set_rec_init(seg_t, conn, port)
    ...
    def _packet_handler(self, msg, session, inport, outport):
        ...
        if outport != ofproto.OFPP_FLOOD:
            if net_layer == 3:
                ...
                if not self._manage_segment(datapath, conn, inport):
                    return
    ...
    def _flow_removed_handler(self, ev):
        ...
        if msg.reason == ofproto.OFPRR_HARD_TIMEOUT:
            ...
            if self.evict_times[dpid] in range(100, 1020, 20):
                with open('evict_tg_{0}.txt'.format(dpid), 'a+') as fp:
    ...
    def _schedule_table(self, datapath, seg):
        ...
        for conn, conn_rec in records.items():
            if self._is_rec_alive(seg, conn):
                port = conn_rec['port']
                alive_rec.setdefault(port, [])
                alive_rec[port].append(conn)
        # calculate Z-score
        num_rules = np.array([len(conns) for conns in alive_rec.values()])
        As = np.sum(num_rules)/len(num_rules)
        div = np.sqrt(np.sum((num_rules - As) ** 2)/len(num_rules))
        Z = (num_rules - As) / div
        # remove the entries with random choice
        for idx, (port, conns) in enumerate(alive_rec.items()):
            if Z[idx] > 0:
                for conn in random.sample(conns, self.batch_size):
