...
class FIFOControllerActive(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(FIFOControllerActive, self).__init__(*args, **kwargs)
    ...
    def _schedule_table(self, datapath, seg):
        ...
        for conn, conn_rec in records.items():
            if self._is_rec_alive(seg, conn):
                alive_rec[conn] = conn_rec['last_time']
        sorted_rec = sorted(alive_rec.items(), key=lambda x: x[1])
        for i in range(min(self.batch_size, len(sorted_rec))):
            conn, last_time = sorted_rec[i]
            # print('conn: {0}, last_time: {1}'.format(conn, last_time))
            self._del_flow(datapath, conn)
            self._set_rec_state(seg, conn, False)
