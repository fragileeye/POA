...
################################################################
    ...
    def _set_rec_init(self, seg, conn, is_mal):
        seg['records'][conn] = {
            'alive': True,
            'is_mal': is_mal,
            'stats': { 
    ...
    def _is_rec_mal(self, seg, conn):
        if conn not in seg['records']:
            return False 
        conn_rec = seg['records'][conn]
        return conn_rec['is_mal']
    ...
    def _manage_segment(self, datapath, conn, is_mal):
        dpid = datapath.id
        ...
        if not self._is_seg_free(seg_p, 'pt'):
            ...
            self._set_rec_init(seg_p, conn, is_mal)
    ...
    def _packet_handler(self, msg, session, inport, outport):
        ...
        if outport != ofproto.OFPP_FLOOD:
            if net_layer == 3:
                ...
                is_mal = True if session[-1] > 10000 else False
                if self._manage_segment(datapath, conn, is_mal):
    ...
    def _flow_removed_handler(self, ev):
        ...
        if msg.reason == ofproto.OFPRR_IDLE_TIMEOUT: 
            ...
            if self._is_rec_mal(seg_p, conn):
                self.evict_mal_times += 1
            self.evict_times += 1
            print('Evict mal rate: {}'.format(self.evict_mal_times/self.evict_times))
            self._dec_seg_ref(seg_p, 'pt')
            self._set_rec_state(seg_p, conn, False)
