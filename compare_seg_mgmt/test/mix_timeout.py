...
class MixController(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(MixController, self).__init__(*args, **kwargs)
    ...
    def _init_vtable(self, dpid, capacity, alpha=0.5, beta=0.5):
        pt_size_t = int(alpha * capacity)
        pt_size_p = int(beta * capacity)

        print('initializing dpid: ', dpid)
        seg_t = self._init_segment(pt_size_t, dpid)
        seg_p = self._init_segment(pt_size_p, dpid)
...
################################################################
    ...
    def _switch_features_handler(self, ev):
        ...
        self._init_vtable(datapath.id, 1000, alpha=0.2, beta=0.8)
