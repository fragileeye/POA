...
class DefenderController(app_manager.RyuApp):
    ...
    def _init_vtable(self, dpid, capacity, alpha=0.9, beta=0.1):
...
################################################################
    ...
    def _query_prob(self, datapath):
        dpid = datapath.id
        seg_p = self.vtables[dpid]['seg_p']
        rate = (seg_p['vt_size'] - seg_p['pt_size']) / 100
        return np.e ** (-rate)

    def _schedule_table(self, datapath, seg):
        # print('schedule table...')
        # get active entries
        prob = self._query_prob(datapath)
