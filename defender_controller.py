...
class DefenderController(app_manager.RyuApp):
    ...
    def _init_vtable(self, dpid, capacity, alpha=0.7, beta=0.2):
        pt_size_t = int(alpha * capacity)
        vt_size_t = capacity
        pt_size_p = int(beta * capacity)
        vt_size_p = pt_size_t * self.delta_lim
...
################################################################
    ...
    def _is_seg_init(self, seg, flag):
        if not seg['init_state']:
            idx = '%s_idx' %flag
            size = '%s_size' %flag
            rate = 1 - self.delta_mon
            if seg[idx] >= rate * seg[size]:
                seg['init_state'] = True
    ...
    def _is_seg_mal(self, seg, flag, chk_size):
        size = '%s_size' %flag
        rate = self.delta_mon
        return chk_size >= rate * seg[size]
    ...
    def _update_segment(self, dpid, conn, stats):
        ...
        if stats is None: # for packet_in events
            if conn in rec_t:
            ...
            else: # the first time trigger packet_in
                conn_rec = {
                    'last_time': 0,
                    't_val': self.init_t0,  # timeout value
                    'alive': True,
                ...
                rec_t[conn] = conn_rec
        else: # for flow_removed events
        # notice the order of assignment between last_time and live_state
            if conn in rec_t:
                self._set_rec_time(seg_t, conn, time.time())
                self._set_rec_state(seg_t, conn, False)
                self._set_rec_stats(seg_t, conn, stats)
            elif conn in rec_p:
                #I'm hesitate if the stats should update 
                self._set_rec_time(seg_p, conn, time.time())
    ...
    def _manage_segment(self, datapath, conn):
        ...
        if conn in rec_p:
            ...
            if not self._is_seg_free(seg_p, 'pt'):
                ...
                self._set_rec_time(seg_p, conn, time.time())
                flag = True
            # malicious event, VPS is overflow.
            if not self._is_seg_free(seg_p, 'vt'):
                print('Time: {0}, dpid: {1}, size: {2} Event: VPS is overflow'.format(time.time(), dpid, seg_p['vt_size']))
            else:
                self._query_classes(seg_p, conn)
                # malicious event, new flow flood.
                if self._is_flow_flood(seg_p):
                    print('Time: {0}, , dpid: {1} Event: new entries flood'.format(time.time(), dpid))
            return flag
    ...
    def _switch_features_handler(self, ev):
        ...
        self._init_vtable(datapath.id, 100)
    ...
    def _query_scores(self, seg, conn):
        records = seg['records']
        update_ival = seg['update_ival']
        # update scores each 10 requests, else use median simi_cr
        if update_ival % self.score_ival == 0:
            ...
            for conn, conn_rec in records.items():
                ...
                conn_rec['score'] = np.log(rate+1) / simi
            seg['simi_cr'] = simi_cr
        else:
            conn_rec = records[conn]
            stats = conn_rec['stats']
            rate = np.mean(stats['pkts'])
            simi = np.median(seg['simi_cr'])
            conn_rec['score'] = np.log(rate+1) / simi
        seg['update_ival'] += 1
    ...
    def _schedule_table(self, datapath, seg):
        ...
        for i in range(self.batch_size):
            cookie, _ = res[i]
            self._del_flow(datapath, cookie)
    ...
    def _query_classes(self, seg, conn):
        ...
        if last_time > 0 and delta_time >= self.detect_ival:
            for _, conn_rec in records.items():
                conn_rec['class'] = 1
            return
        # not initialized, the entries should be valid.
        ...
        if len(new_rec) >= self.batch_size:
            ...
            result = self.handler.check_drift(old_ds, new_ds)
            class_value = 1 if result == 0 else -1
            for conn, conn_rec in records.items():
                if conn_rec['class'] == 0:
                    conn_rec['class'] = class_value
                        
    def _is_flow_flood(self, seg):
        records = seg['records']
        unk_size = 0
        for _, conn_rec in records.items():
            if conn_rec['class'] == -1:
                unk_size += 1
        return self._is_seg_mal(seg, 'pt', unk_size)
